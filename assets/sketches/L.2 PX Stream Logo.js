...
let phases = []
let speeds = []
let missileStates = [] // 0=spiral in, 1=at logo, 2=falling
let fallPhases = []
let spiralRadii = []
let spiralAngles = []
...
function setup () {
  ...
  for (let layer = 0; layer < D; layer++) {
    for (let i = 0; i < p.length; i += 2) {
      ...
      speeds.push(random(0.01, 0.03))
      spiralRadii.push(random(SPIRAL_RADIUS * 0.8, SPIRAL_RADIUS * 1.5))
      spiralAngles.push(random(TAU))

      // State management
      missileStates.push(random() > 0.3 ? 1 : 0) // most start at logo position
      fallPhases.push(0)
    }
  }
}
...
function draw () {
  ...
  for (let i = 0; i < base.length; i++) {
    let set = D - Math.floor(i / (p.length / 2))

    // Set stroke alpha per layer
    if (set !== prevSet) {
      prevSet = set
      stroke(255, 255 / ((set * .5) + 1))
    }

    let [bx, by] = base[i]
    let phase = phases[i]
    let spd = speeds[i]
    let state = missileStates[i]

    let x, y

    if (state === 0) {
      // Spiral in like a missile
      let spiralProgress = (t * spd + phase) % TAU
      let radius = spiralRadii[i] * (1 - spiralProgress / TAU)
      let angle = spiralAngles[i] + spiralProgress * 3

      x = bx + cos(angle) * radius
      y = by + sin(angle) * radius

      // Transition to logo position when spiral is complete
      if (radius < 5) {
        missileStates[i] = 1
        fallPhases[i] = 0
      }
    } else if (state === 1) {
      // At logo position with slight jitter
      let dx = sin(t * spd + phase) * T * 0.5
      let dy = cos(t * spd * 1.3 + phase * 1.7) * T * 0.3

      x = bx + dx
      y = by + dy

      // Randomly start falling
      if (random() < FALL_CHANCE) {
        missileStates[i] = 2
        fallPhases[i] = 0
      }
    } else if (state === 2) {
      // Falling down
      fallPhases[i] += RETURN_SPEED
      let fallY = by + fallPhases[i] * fallPhases[i] * 200
      let fallX = bx + sin(fallPhases[i] * 5) * 10 // slight wobble

      x = fallX
      y = fallY

      // Reset to spiral when off screen
      if (y > height + 50) {
        missileStates[i] = 0
        spiralAngles[i] = random(TAU)
        spiralRadii[i] = random(SPIRAL_RADIUS * 0.8, SPIRAL_RADIUS * 1.5)
        phases[i] = random(TAU)
      }
    }
