/*{
    ...
    "INPUTS": [
        ...
        {"NAME": "SPEED", "TYPE": "float", "DEFAULT": 0.25, "MIN": 0, "MAX": 10},
        {"NAME": "FADE_IN", "TYPE": "float", "DEFAULT": 1, "MIN": 0, "MAX": 10},
        {"NAME": "start", "TYPE": "float", "DEFAULT": 9, "MIN": 0, "MAX": 24},
        {"NAME": "MOON", "TYPE": "float", "DEFAULT": 1, "MIN": 0, "MAX": 1}
    ],
    "ISFVSN": "2",
    "DESCRIPTION": "Adapted from https://www.shadertoy.com/view/7dyyRy"
}*/
...
#define tanh_approx(x) tanh(clamp(x, -10.,10.))

// License: Unknown, author: Unknown, found: don't remember
// float tanh_approx(float x_) {
...
vec3 stars(vec2 sp, float hh) {
    ...
    for (float i = 0.0; i < m; ++i) {
        ...
        ccol *= mix(0.125, 1.0, smoothstep(1.0, 0.99, sin(SPEED*TIME+TAU*h.y)));
        col = h3 < y ? ccol : col;
    }
...
vec4 moon(vec3 ro, vec3 rd) {
    ...
    for (int i = -limit; i <= limit; ++i) {
        ...
        float d0    = abs(pp.y);
        vec2 cp     = vec2(0.055*abs(msny-float(i)), 0.5);
        //float fft   = texture(iChannel0, cp).x;
        float fft = IMG_NORM_PIXEL(fftImage, cp).x;
...
vec3 skyColor(vec3 ro, vec3 rd) {
    ...
    col  = mix(col, mcol.xyz, mcol.w * MOON);
...
void main() {
    ...
    vec3 col = vec3(0.0);
    col = effect(p, q);
    col *= smoothstep(0.0, 8.0, TIME*FADE_IN-abs(q.y));
    col = aces_approx(col);
    col = sRGB(col);
