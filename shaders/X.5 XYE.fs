/*{
    ...
    "ISFVSN": "2",
    "DESCRIPTION": "XY Oscilloscope, extended electron emulation"
}*/
...
void main() {
    ...
    if(PASSINDEX == 0) {
        ...
        vec3 newSignal = (uv.x >= 0. && uv.x <= 1.) ? vec3(IMG_NORM_PIXEL(xyeImage, uv).r) : vec3(0);
        ...
        for(float x = -range; x <= range; x += 1.0) {
            for(float y = -range; y <= range; y += 1.0) {
                ...
                if(sampleUV.x >= 0. && sampleUV.x <= 1. && sampleUV.y >= 0. && sampleUV.y <= 1.) {
                    ...
                    vec3 sample = vec3(IMG_NORM_PIXEL(xyeImage, sampleUV).r);
        ...
        vec3 phosphorColor = color.r > threshold ? fg.rgb : bg.rgb;
        color *= phosphorColor;
